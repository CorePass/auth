...
import {CorePassError} from './errors';
    ...
    return new Promise<T>((resolve, reject) => {
      xhr.then(
          ...
          (jqXhr, textStatus, error) => {
            reject(new CorePassError(error));
          });
    });
  }
}
...
export class FetchRequestor extends Requestor {
  xhr<T>(settings: JQueryAjaxSettings): Promise<T> {
    if (!settings.url) {
      return Promise.reject(new CorePassError('A URL must be provided.'));
    }
    ...
    return fetch(url.toString(), requestInit).then(response => {
      if (response.status >= 200 && response.status < 300) {
        ...
        return Promise.reject(new CorePassError(response.status.toString(), response.statusText));
      }
    });
  }
}
